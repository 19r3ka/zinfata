app.controller('albumCtrl', ['$scope', '$rootScope', '$location', '$routeParams', 'SessionSvc', 'TracksSvc', 'QueueSvc', 'AlbumsSvc', 'UsersSvc', 'MessageSvc', 'ALBUM_EVENTS', '$log',
    ...
    if($routeParams.albumId){
        AlbumsSvc.get($routeParams.albumId, function(data) {
            ...
            if(!!$scope.album.artist.id && ($scope.album.artist.id === Session.getCurrentUser()._id)) $scope.canEdit = true;
            ...
            Tracks.find({a_id: data._id} , function(tracks) {
                angular.forEach(tracks, function(track) {
                    ...
                    if(angular.isNumber(duration)) $scope.album.duration += duration;
                });
            }, function(err) {});
    	}, function(err) {
    		$location.path('/');
    ...
    $scope.create = function(album) {
        if(!album.artist.id) album.artist.id = Session.getCurrentUser()._id;
    ...
    $scope.update = function(album) {
        if($scope.album.duration)    delete $scope.album.duration;
        if($scope.album.trackLength) delete $scope.album.trackLength;
        if(!album.artist.id) album.artist.id = Session.getCurrentUser()._id;
