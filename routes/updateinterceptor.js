...
module.exports = function(wagner) {
  function checkAccess(loggedUserId, ownerId, route, method, next) {
    ...
    wagner.invoke(function(ZError) {
      if (String(loggedUserId) === String(ownerId)) { // id match
        ...
        return next(new ZError('forbidden',
          'You are not the album owner. ' +
          'Only the ' + route + ' owner can ' + method + ' it'));
      }
    });
  }
  ...
  return function(req, res, next) {
    ...
    if (req.params.id && parseInt(req.params.id) !== 0) {
      ...
      if (bearer) {
        wagner.invoke(
        function(OAuthAccessToken, User, Album, Track, Playlist, ZError) {
                ...
                switch (route) {
                    ...
                    Album.findById(id, function(err, album) {
                      ...
                      if (!album) {
                        return next(new ZError('not_found',
                          'The album to ' + method + ' was not found'));
                      }
                  ...
                  case 'playlists':
                    Playlist.findById(id, function(err, playlist) {
                      ...
                      if (!playlist) {
                        return next(new ZError('not_found',
                          'The playlist to ' + method + ' was not found'));
                      }
                  ...
                  case 'users':
                    ...
                    User.findById(id, function(err, user) {
                      ...
                      if (!user) {
                        return next(new ZError('not_found',
                          'The user  to ' + method + ' was not found'));
                      }
                      ownerId = user._id;
                      checkAccess(loggedUserId, ownerId, route, method, next);
                    });
