...
var OAuthAccessTokenSchema = new mongoose.Schema({
  ...
  clientId:     {type: String, required: true, ref: 'OAuthClient.clientId',
                lowercase: true},
...
OAuthAccessTokenSchema.pre('save', function(next) {
  ...
  OAuthClient.findOne({clientId: self.clientId}, function(err, client) {
    ...
    if (!client) {
      return next(new Error('Invalid clientId'));
    }
...
OAuthAccessTokenSchema.statics.getAccessToken =
  function(bearerToken, callback) {
  console.log('in getAccessToken (bearerToken: ' + bearerToken + ')');
  OAuthAccessTokenModel.findOne({accessToken: bearerToken}, callback);
};

OAuthAccessTokenSchema.statics.saveAccessToken =
  function(token, clientId, expires, userId, callback) {
...
OAuthAccessTokenSchema.statics.revokeAccessToken =
  function(accessToken, callback) {
  console.log('in revokeAccessToken (accessToken: ' + accessToken + ')');
  OAuthAccessTokenModel.findOne({accessToken: accessToken},
